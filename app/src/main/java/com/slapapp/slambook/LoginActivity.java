...
public class LoginActivity extends AppCompatActivity {
    ...
    private void attemptRegister() {
            cancel=false;
//            if(mEmailView.getText().length()<4||isEmailValid(mEmailView.getText().toString())){
//                mEmailView.setError("Give Valid Email Address");
//                cancel= true;
//            }
        ...
        if(!cancel){
            ...
            //editor.putString("logFor",emailvalue);
            editor.commit();
    ...
    private void attemptLogin() {
        ...
        if (cancel) {
            ...
            if(!mEmailView.getText().toString().equals(vEmail)||!mPasswordView.getText().toString().equals(vPass)
                    ||vPass.equals("")||vEmail.equals("")){
                Toast.makeText(getApplicationContext(),"Invalid UserName or Password Can't Login",Toast.LENGTH_LONG).show();
            }
            else {
               /* SharedPreferences.Editor editor = sharedpreferences.edit();
                editor.putBoolean("isLogin",true);
                editor.putString("logFor",vEmail);
                editor.commit();*/
